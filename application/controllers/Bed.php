...
class Bed extends CI_Controller {
    ...
    function manage_bed($param1 = null, $param2 = null, $param3 = null ){
        ...
        if($param1 == 'create'){
            ...
            $this->session->set_flashdata('flash_message', get_phrase('Bed Added Successfully'));
            redirect(base_url(). 'bed/manage_bed', 'refresh');            
        }

        if($param1 == 'update'){
            $this->bed_model->updateBedInfo($param2);
            $this->session->set_flashdata('flash_message', get_phrase('Bed Updated Successfully'));
            redirect(base_url(). 'bed/manage_bed', 'refresh');            
        }

        if($param1 == 'delete'){
            $this->bed_model->deleteFromBedTable($param2);
            $this->session->set_flashdata('flash_message', get_phrase('Bed Deleted Successfully'));
            redirect(base_url(). 'bed/manage_bed', 'refresh');            
        }
    ...
    function assign_bed($param1 = null, $param2 = null, $param3 = null ){
        ...
        if($param1 == 'create'){
            ...
            $this->session->set_flashdata('flash_message', get_phrase('Bed Assigned Successfully'));
            redirect(base_url(). 'bed/assign_bed', 'refresh');            
        }

        if($param1 == 'update'){
            $this->bed_model->updateAssignBedTable($param2);
            $this->session->set_flashdata('flash_message', get_phrase('Assigned Bed Updated Successfully'));
            redirect(base_url(). 'bed/assign_bed', 'refresh');            
        }

        if($param1 == 'delete'){
            $this->bed_model->deleteAssignBedInformation($param2);
            $this->session->set_flashdata('flash_message', get_phrase('Assigned Bed Deleted Successfully'));
            redirect(base_url(). 'bed/assign_bed', 'refresh');            
        }
    ...
    function display_bed_brief_info ($bed_id){
        ...
        foreach ($select_beds as $key => $bed){
            //This displays the bed details, it is controlled by the ajax JS function in the assign_bed.php file
        echo '<hr>';
        echo ''.'<b>Bed Name:</b> '.$bed['name'].' <br><b>Bed Price:</b> '.$bed['bed_price'].' <br><b>Bed Size:</b> '.'"'.$bed['bed_size'].'"';
        echo '<hr>';
        }
    }
