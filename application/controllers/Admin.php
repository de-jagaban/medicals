<?php if (!defined('BASEPATH'))exit('No direct script access allowed'); //This ensures the route is following the basepath


class Admin extends CI_Controller {

    function __construct() {
        parent::__construct();

		$this->load->database(); //This loads the database
		$this->load->library('session'); //To track user activities
        $this->load->model('department_model');
        $this->load->model('doctor_model');
        $this->load->model('nurse_model');
        $this->load->model('accountant_model');
        $this->load->model('pharmacist_model');
        $this->load->model('receptionist_model');
        $this->load->model('laboratorist_model');
        $this->load->model('generalService_model');
        $this->load->model('schedule_model');
        $this->load->model('medicine_model');
        $this->load->model('notification_model');
       
        
    }


    public function index() {

        if($this->session->userdata('admin_login') != 1) redirect(base_url(). 'login', 'refresh'); //Redirects user to login page on clicking logout
        if($this->session->userdata('admin_login') == 1) redirect(base_url(). 'admin/dashboard', 'refresh'); //checks if user is already logged in
       


    }

    function dashboard(){

        if($this->session->userdata('admin_login') != 1) redirect(base_url(). 'login', 'refresh');

        //This method calls the Admin Dashboard
        $page_data['page_name']     = 'dashboard'; //This loads the page name from the view controller.
        $page_data['page_title']    = get_phrase('Admin Dashboard'); //This loads the page title

        $this->load->view('backend/index', $page_data);
    }

    //****** This Function controls the Admin Profile Change Settings */
    function change_profile($param1 = null, $param2 = null, $param3 = null ){

        if($param1 == 'update_info'){
            $this->admin_model->updateAdminInfoFunction();
            $this->session->set_flashdata('flash_message', get_phrase('Data Updated Successfully'));
            redirect(base_url(). 'admin/change_profile', 'refresh');

            
        }

        if($param1 == 'change_password'){
            $this->admin_model->changePasswordFunction();
            $this->session->set_flashdata('flash_message', get_phrase('Password Changed Successfully'));
            redirect(base_url(). 'admin/change_profile', 'refresh');


        }

        $page_data['page_name']     = 'change_profile'; //This loads the page name from the view controller.
        $page_data['page_title']    = get_phrase('Change Profile'); //This loads the page title

        $this->load->view('backend/index', $page_data);

    }
    //*** Ends Here */


    //******** These functions Manage the hospital doctor **********/
    function manage_doctor($param1 = null, $param2 = null, $param3 = null){


        if($param1 == 'create'){
            $this->doctor_model->insertIntoDoctorTable();
            $this->session->set_flashdata('flash_message', get_phrase('Data Saved Successfully'));
            redirect(base_url(). 'admin/manage_doctor', 'refresh');            
        }
       
        if($param1 == 'update'){
            $this->doctor_model->updateDoctorInformation($param2);
            $this->session->set_flashdata('flash_message', get_phrase('Data Updated Successfully'));
            redirect(base_url(). 'admin/manage_doctor', 'refresh');             
        }

        if($param1 == 'delete'){
            $this->doctor_model->deleteDoctorInformation($param2);
            $this->session->set_flashdata('flash_message', get_phrase('Data Deleted Successfully'));
            redirect(base_url(). 'admin/manage_doctor', 'refresh');            
        }

        $page_data['page_name'] = 'manage_doctor';
        $page_data['page_title'] = get_phrase('Manage Doctor');
        $this->load->view('backend/index', $page_data);
    }
    //******** Ends Here  */


    //******** This function displays the doctor's profile **********/
    function completeDoctorProfile($doctor_id){
        $page_data['doctor_profile'] = $this->db->get_where('doctor', array('doctor_id' => $doctor_id))->result_array();
        $page_data['page_name'] = 'completeDoctorProfile';
        $page_data['page_title'] = get_phrase('Doctor Profile');
        $this->load->view('backend/index', $page_data);



    }


     //******** These functions Manage the hospital nurse **********/
     function manage_nurse($param1 = null, $param2 = null, $param3 = null){


        if($param1 == 'create'){
            $this->nurse_model->insertIntoNurseTable();
            $this->session->set_flashdata('flash_message', get_phrase('Data Saved Successfully'));
            redirect(base_url(). 'admin/manage_nurse', 'refresh');            
        }
       
        if($param1 == 'update'){
            $this->nurse_model->updateNurseInformation($param2);
            $this->session->set_flashdata('flash_message', get_phrase('Data Updated Successfully'));
            redirect(base_url(). 'admin/manage_nurse', 'refresh');             
        }

        if($param1 == 'delete'){
            $this->nurse_model->deleteNurseInformation($param2);
            $this->session->set_flashdata('flash_message', get_phrase('Data Deleted Successfully'));
            redirect(base_url(). 'admin/manage_nurse', 'refresh');            
        }

        $page_data['page_name'] = 'manage_nurse';
        $page_data['page_title'] = get_phrase('Manage nurse');
        $this->load->view('backend/index', $page_data);
    }
    //******** Ends Here  */


    //******** This function displays the nurse's profile **********/
    function completeNurseProfile($nurse_id){
        $page_data['nurse_profile'] = $this->db->get_where('nurse', array('nurse_id' => $nurse_id))->result_array();
        $page_data['page_name'] = 'completeNurseProfile';
        $page_data['page_title'] = get_phrase('nurse Profile');
        $this->load->view('backend/index', $page_data);



    }

    
    //******** These functions Manage the hospital accountant **********/
    function manage_accountant($param1 = null, $param2 = null, $param3 = null){


        if($param1 == 'create'){
            $this->accountant_model->insertIntoAccountantTable();
            $this->session->set_flashdata('flash_message', get_phrase('Data Saved Successfully'));
            redirect(base_url(). 'admin/manage_accountant', 'refresh');            
        }
       
        if($param1 == 'update'){
            $this->accountant_model->updateAccountantInformation($param2);
            $this->session->set_flashdata('flash_message', get_phrase('Data Updated Successfully'));
            redirect(base_url(). 'admin/manage_accountant', 'refresh');             
        }

        if($param1 == 'delete'){
            $this->accountant_model->deleteAccountantInformation($param2);
            $this->session->set_flashdata('flash_message', get_phrase('Data Deleted Successfully'));
            redirect(base_url(). 'admin/manage_accountant', 'refresh');            
        }

        $page_data['page_name'] = 'manage_accountant';
        $page_data['page_title'] = get_phrase('Manage accountant');
        $this->load->view('backend/index', $page_data);
    }
    //******** Ends Here  */


    //******** This function displays the accountant's profile **********/
    function completeAccountantProfile($accountant_id){
        $page_data['accountant_profile'] = $this->db->get_where('accountant', array('accountant_id' => $accountant_id))->result_array();
        $page_data['page_name'] = 'completeAccountantProfile';
        $page_data['page_title'] = get_phrase('accountant Profile');
        $this->load->view('backend/index', $page_data);



    }


    //******** These functions Manage the hospital pharmacist **********/
    function manage_pharmacist($param1 = null, $param2 = null, $param3 = null){


        if($param1 == 'create'){
            $this->pharmacist_model->insertIntoPharmacistTable();
            $this->session->set_flashdata('flash_message', get_phrase('Data Saved Successfully'));
            redirect(base_url(). 'admin/manage_pharmacist', 'refresh');            
        }
       
        if($param1 == 'update'){
            $this->pharmacist_model->updatePharmacistInformation($param2);
            $this->session->set_flashdata('flash_message', get_phrase('Data Updated Successfully'));
            redirect(base_url(). 'admin/manage_pharmacist', 'refresh');             
        }

        if($param1 == 'delete'){
            $this->pharmacist_model->deletePharmacistInformation($param2);
            $this->session->set_flashdata('flash_message', get_phrase('Data Deleted Successfully'));
            redirect(base_url(). 'admin/manage_pharmacist', 'refresh');            
        }

        $page_data['page_name'] = 'manage_pharmacist';
        $page_data['page_title'] = get_phrase('Manage Pharmacist');
        $this->load->view('backend/index', $page_data);
    }
    //******** Ends Here  */


    //******** This function displays the pharmacist's profile **********/
    function completePharmacistProfile($pharmacist_id){
        $page_data['pharmacist_profile'] = $this->db->get_where('pharmacist', array('pharmacist_id' => $pharmacist_id))->result_array();
        $page_data['page_name'] = 'completePharmacistProfile';
        $page_data['page_title'] = get_phrase('pharmacist Profile');
        $this->load->view('backend/index', $page_data);



    }


    //******** These functions Manage the hospital receptionist **********/
    function manage_receptionist($param1 = null, $param2 = null, $param3 = null){


        if($param1 == 'create'){
            $this->receptionist_model->insertIntoReceptionistTable();
            $this->session->set_flashdata('flash_message', get_phrase('Receptionist Added Successfully'));
            redirect(base_url(). 'admin/manage_receptionist', 'refresh');            
        }
       
        if($param1 == 'update'){
            $this->receptionist_model->updateReceptionistInformation($param2);
            $this->session->set_flashdata('flash_message', get_phrase('Data Updated Successfully'));
            redirect(base_url(). 'admin/manage_receptionist', 'refresh');             
        }

        if($param1 == 'delete'){
            $this->receptionist_model->deleteReceptionistInformation($param2);
            $this->session->set_flashdata('flash_message', get_phrase('Receptionist Deleted Successfully'));
            redirect(base_url(). 'admin/manage_receptionist', 'refresh');            
        }

        $page_data['page_name'] = 'manage_receptionist';
        $page_data['page_title'] = get_phrase('Manage Receptionist');
        $this->load->view('backend/index', $page_data);
    }
    //******** Ends Here  */


    //******** This function displays the receptionist's profile **********/
    function completeReceptionistProfile($receptionist_id){
        $page_data['receptionist_profile'] = $this->db->get_where('receptionist', array('receptionist_id' => $receptionist_id))->result_array();
        $page_data['page_name'] = 'completeReceptionistProfile';
        $page_data['page_title'] = get_phrase('receptionist Profile');
        $this->load->view('backend/index', $page_data);


    }


    //******** These functions Manage the hospital laboratorist **********/
    function manage_laboratorist($param1 = null, $param2 = null, $param3 = null){


        if($param1 == 'create'){
            $this->laboratorist_model->insertIntoLaboratoristTable();
            $this->session->set_flashdata('flash_message', get_phrase('Data Saved Successfully'));
            redirect(base_url(). 'admin/manage_laboratorist', 'refresh');            
        }
       
        if($param1 == 'update'){
            $this->laboratorist_model->updateLaboratoristInformation($param2);
            $this->session->set_flashdata('flash_message', get_phrase('Data Updated Successfully'));
            redirect(base_url(). 'admin/manage_laboratorist', 'refresh');             
        }

        if($param1 == 'delete'){
            $this->laboratorist_model->deleteLaboratoristInformation($param2);
            $this->session->set_flashdata('flash_message', get_phrase('Data Deleted Successfully'));
            redirect(base_url(). 'admin/manage_laboratorist', 'refresh');            
        }

        $page_data['page_name'] = 'manage_laboratorist';
        $page_data['page_title'] = get_phrase('Manage Laboratorist');
        $this->load->view('backend/index', $page_data);
    }
    //******** Ends Here  */


    //******** This function displays the laboratorist's profile **********/
    function completeLaboratoristProfile($laboratorist_id){
        $page_data['laboratorist_profile'] = $this->db->get_where('laboratorist', array('laboratorist_id' => $laboratorist_id))->result_array();
        $page_data['page_name'] = 'completeLaboratoristProfile';
        $page_data['page_title'] = get_phrase('Laboratorist Profile');
        $this->load->view('backend/index', $page_data);


    }


    //******** These functions Manage the hospital generalService **********/
    function manage_generalService($param1 = null, $param2 = null, $param3 = null){


        if($param1 == 'create'){
            $this->generalService_model->insertIntoGeneralServiceTable();
            $this->session->set_flashdata('flash_message', get_phrase('Data Saved Successfully'));
            redirect(base_url(). 'admin/manage_generalService', 'refresh');            
        }
       
        if($param1 == 'update'){
            $this->generalService_model->updateGeneralServiceInformation($param2);
            $this->session->set_flashdata('flash_message', get_phrase('Data Updated Successfully'));
            redirect(base_url(). 'admin/manage_generalService', 'refresh');             
        }

        if($param1 == 'delete'){
            $this->generalService_model->deleteGeneralServiceInformation($param2);
            $this->session->set_flashdata('flash_message', get_phrase('Data Deleted Successfully'));
            redirect(base_url(). 'admin/manage_generalService', 'refresh');            
        }

        $page_data['page_name'] = 'manage_generalService';
        $page_data['page_title'] = get_phrase('Manage General Services');
        $this->load->view('backend/index', $page_data);
    }
    //******** Ends Here  */


    //******** This function displays the generalService's profile **********/
    function completeGeneralServiceProfile($generalService_id){
        $page_data['generalService_profile'] = $this->db->get_where('generalService', array('generalService_id' => $generalService_id))->result_array();
        $page_data['page_name'] = 'completeGeneralServiceProfile';
        $page_data['page_title'] = get_phrase('General Service Profile');
        $this->load->view('backend/index', $page_data);


    }

    function add_schedule($param1 = null, $param2 = null, $param3 = null){

        if($param1 == 'create'){
           $this->schedule_model->insertIntoScheduleTable();
           $this->session->set_flashdata('flash_message', get_phrase('Schedule Created Successfully'));
            redirect(base_url(). 'admin/list_schedule', 'refresh'); 
                      
        }


        if($param1 == 'update'){
             $this->schedule_model->updateScheduleTable($param2);             
             $this->session->set_flashdata('flash_message', get_phrase('Schedule updated Successfully'));
             redirect(base_url(). 'admin/list_schedule', 'refresh'); 
           
 
            }


         if($param1 == 'delete'){
             $this->schedule_model->deleteScheduleTable($param2);
             $this->session->set_flashdata('flash_message', get_phrase('Schedule deleted Successfully'));
             redirect(base_url(). 'admin/list_schedule', 'refresh'); 
            }

        $page_data['page_name'] = 'add_schedule';
        $page_data['page_title'] = get_phrase('Add Schedule');
        $this->load->view('backend/index', $page_data);
    }


    function list_schedule(){

        $page_data['page_name'] = 'list_schedule';
        $page_data['page_title'] = get_phrase('List Schedule');
        $this->load->view('backend/index', $page_data);
    }

    function get_department_doctor($department_id){
        //This function works with the Ajax script function in the add_schedule.php 
        //and autoloads the list of doctors based on the selected department

        $doctors = $this->db->get_where('doctor', array('department_id' => $department_id))->result_array();
        foreach($doctors as $key => $doctors){
            echo '<option value="'.$doctors['doctor_id'].'">'.$doctors['name'].'</option>';
        }
    }

    function edit_schedule($schedule_id){

        $page_data['get_schedule'] = $this->schedule_model->get_schedule_by_id($schedule_id);
        $page_data['page_name'] = 'edit_schedule';
        $page_data['page_title'] = get_phrase('Edit Schedule');
        $this->load->view('backend/index', $page_data);


    }


    function add_appointment ($param1 = null, $param2 = null, $param3 = null){

        if($param1 == 'create'){

            $this->appointment_model->insertIntoAppointmentTable();
            $this->session->set_flashdata('flash_message', get_phrase('Appointment Created Successfully'));
            redirect(base_url() . 'admin/list_appointment', 'refresh');
        }

        if($param1 == 'update'){

            $this->appointment_model->updateAppointmentInformation($param2);
            $this->session->set_flashdata('flash_message', get_phrase('Appointment Updated Successfully'));
            redirect(base_url() . 'admin/list_appointment', 'refresh');
        }

        if($param1 == 'delete'){

            $this->appointment_model->deleteAppointmentInformation($param2);
            $this->session->set_flashdata('flash_message', get_phrase('Appointment Deleted Successfully'));
            redirect(base_url() . 'admin/list_appointment', 'refresh');
        }

        $page_data['page_name'] =   'add_appointment';
        $page_data['page_title'] =   get_phrase('Add Appointment');
        $this->load->view('backend/index', $page_data);

    }


    function select_doctor_patient($department_id){
        //This function controls the automatic display of the doctor and patient in a particular department
        //when the department is selected.

        $page_data['department_id'] = $department_id;
        $this->load->view('backend/admin/display_doctor_patient', $page_data);
        //The loaded view above is automatic loaded once the department is selected and the JS Ajax function
        //in add_appointment.php is initiated.
    }

    function get_doctor_schedule($doctor_id){
        $schedules = $this->db->get_where('schedule', array('doctor_id' => $doctor_id))->result_array();
        foreach ($schedules as $key => $schedule){
            echo '<option value="'.$schedule['schedule_id'].'">'.'Schedule Date: '. date('d M Y', $schedule['avail_day']) .' - '.'Start Time: '.$schedule['start_time'].' End Time: '.$schedule['end_time'].'</option>';
        }
    }


    function get_department_patient($department_id){
        $patients = $this->db->get_where('patient', array('department_id' => $department_id))->result_array();
        foreach ($patients as $key => $patient){
            echo '<option value="'.$patient['patient_id'].'">'.$patient['name'].'</option>';
        }

    }


    function list_appointment (){
        $page_data['page_name'] =   'list_appointment';
        $page_data['page_title'] =   get_phrase('List Appointment');
        $this->load->view('backend/index', $page_data);
    }

    function edit_appointment ($appointment_id){


        $page_data['display_appointment'] = $this->appointment_model->get_appointment_by_id($appointment_id);
        $page_data['appointment_id'] = $appointment_id;
        $page_data['page_name'] =   'edit_appointment';
        $page_data['page_title'] =   get_phrase('Edit Appointment');
        $this->load->view('backend/index', $page_data);
    }


    function get_doctor_patient_edit ($department_id, $appointment_id){

        $page_data['department_id'] = $department_id;
        $page_data['appointment_id'] = $appointment_id;
        $this->load->view('backend/admin/display_doctor_patient_edit', $page_data); //This displays the doctor and patient 
    }


    function medicine_category ($param1 = null, $param2 = null, $param3 = null){

        if($param1 == 'create'){

            $this->medicine_model->insertIntoMedicineCategoryTable();
            $this->session->set_flashdata('flash_message', get_phrase('Med-Category Added Successfully'));
            redirect(base_url() . 'admin/medicine_category', 'refresh');
        }

        if($param1 == 'update'){

            $this->medicine_model->updateMedicineCategory($param2);
            $this->session->set_flashdata('flash_message', get_phrase('Med-Category Updated Successfully'));
            redirect(base_url() . 'admin/medicine_category', 'refresh');
        }

        if($param1 == 'delete'){

            $this->medicine_model->deleteMedicineCategory($param2);
            $this->session->set_flashdata('flash_message', get_phrase('Med-Category Deleted Successfully'));
            redirect(base_url() . 'admin/medicine_category', 'refresh');
        }

        $page_data['page_name'] =   'medicine_category';
        $page_data['page_title'] =   get_phrase('Add Medicine Category');
        $this->load->view('backend/index', $page_data);

    }


    function manage_medicine ($param1 = null, $param2 = null, $param3 = null){

        if($param1 == 'create'){

            $this->medicine_model->insertIntoMedicineTable();
            $this->session->set_flashdata('flash_message', get_phrase('Medicine Saved Successfully'));
            redirect(base_url() . 'admin/manage_medicine', 'refresh');
        }

        if($param1 == 'update'){

            $this->medicine_model->updateMedicineTable($param2);
            $this->session->set_flashdata('flash_message', get_phrase('Medicine Updated Successfully'));
            redirect(base_url() . 'admin/manage_medicine', 'refresh');
        }

        if($param1 == 'delete'){

            $this->medicine_model->deleteFromMedicineTable($param2);
            $this->session->set_flashdata('flash_message', get_phrase('Medicine Deleted Successfully'));
            redirect(base_url() . 'admin/manage_medicine', 'refresh');
        }

        $page_data['page_name'] =   'manage_medicine';
        $page_data['page_title'] =   get_phrase('Add Medicine');
        $this->load->view('backend/index', $page_data);

    }

    
    function set_language($lang){
        $this->session->set_userdata('language', $lang);
        redirect(base_url() . 'admin', 'refresh');
        recache();
    }


    function saveGeneralMessage(){

        $page_data['message'] = $this->input->post('chatSend');
        $page_data['user_id'] = $this->input->post('user_id');
        
        $sql = "select * from general_message order by general_message_id desc limit 1";
        $return_query = $this->db->query($sql)->row()->general_message_id + 1;
        $page_data['general_message_id'] = $return_query;
            
        $this->db->insert('general_message', $page_data);
        echo json_encode($page_data);
    }



    function notification ($param1 = null, $param2 = null, $param3 = null){

        if($param1 == 'create'){

            $this->notification_model->insertIntoNotificationTable();
            $this->session->set_flashdata('flash_message', get_phrase('Event Added Successfully'));
            redirect(base_url() . 'admin/notification', 'refresh');
        }

        if($param1 == 'update'){

            $this->notification_model->updateNotificationTable($param2);
            $this->session->set_flashdata('flash_message', get_phrase('Event Updated Successfully'));
            redirect(base_url() . 'admin/notification', 'refresh');
        }

        if($param1 == 'delete'){

            $this->notification_model->deleteFromNotificationTable($param2);
            $this->session->set_flashdata('flash_message', get_phrase('Event Deleted Successfully'));
            redirect(base_url() . 'admin/notification', 'refresh');
        }

        $page_data['page_name'] =   'notification';
        $page_data['page_title'] =   get_phrase('Add Event');
        $this->load->view('backend/index', $page_data);

    }



}
