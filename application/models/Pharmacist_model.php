...
class Pharmacist_model extends CI_Model {
    ...
    function __construct() {
      ...
      //This function selects the pharmacist from the pharmacist table in the database.
      function select_all_pharmacists(){
    ...
    /*ends here */
     ...
     //This will insert the data entered in the form into the pharmacist table.
    function insertIntoPharmacistTable(){   
        ...
        //Email validation
        $check_email = $this->db->get_where('pharmacist', array('email' => $page_data['email']))->row()->email;
        if($check_email != null || $check_email != ''){
            ...
            $this->db->insert('pharmacist', $page_data); //Inserts into the database.
            ...
            //Uploads pharmacist's image into pharmacist image folder
     ...
     //This will update the data entered in the form into the pharmacist database.
    function updatePharmacistInformation($param2){      
        $page_data['name']              = html_escape($this->input->post('name'));
        $page_data['date_of_birth']     = strtotime($this->input->post('date_of_birth'));
        ...
        $this->db->where('pharmacist_id', $param2); //Will delete based on the department_id
        $this->db->update('pharmacist', $page_data); //Updates the database.

        move_uploaded_file($_FILES['userfile']['tmp_name'], 'uploads/pharmacist_image/' . $param2 . '.jpg');

    }

    function deletePharmacistInformation($param2){
        //This will delete the data entered in the form into the department database.
        $this->db->where('pharmacist_id', $param2); //Will delete based on the department_id
        $this->db->delete('pharmacist'); //Updates the database.
    }

    
    function updatePharmacistInfoFunction(){
        ...
        $this->db->update('pharmacist', $page_data);
    ...
    function changePasswordFunction (){
        ...
        if($page_data['password'] == $confirm_password){
            ...
            redirect(base_url() . 'pharmacist/change_profile', 'refresh');
