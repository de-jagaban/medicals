...
class Nurse_model extends CI_Model {
    ...
    function __construct() {
      ...
      //This function selects the nurse from the nurse table in the database.
    ...
    /*ends here */
     ...
     //This will insert the data entered in the form into the nurse table.
    function insertIntoNurseTable(){   
        ...
        if($check_email != null || $check_email != ''){
            ...
            $this->db->insert('nurse', $page_data); //Inserts into the database.
     ...
     //This will update the data entered in the form into the nurse database.
    function updateNurseInformation($param2){      
        ...
        $this->db->where('nurse_id', $param2); //Will delete based on the department_id
        $this->db->update('nurse', $page_data); //Updates the database.

        move_uploaded_file($_FILES['userfile']['tmp_name'], 'uploads/nurse_image/' . $param2 . '.jpg');

    }

    function deleteNurseInformation($param2){
        //This will delete the data entered in the form into the department database.
        $this->db->where('nurse_id', $param2); //Will delete based on the department_id
        $this->db->delete('nurse'); //Updates the database.
    }
