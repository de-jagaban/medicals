...
class GeneralService_model extends CI_Model {
    ...
    function __construct() {
      ...
      //This function selects the generalService from the generalService table in the database.
      function select_all_generalServices(){
        $query = $this->db->get('generalService');
    ...
    /*ends here */
     ...
     //This will insert the data entered in the form into the generalService table.
    function insertIntoGeneralServiceTable(){   
        ...
        //Email validation
        $check_email = $this->db->get_where('generalService', array('email' => $page_data['email']))->row()->email;
        if($check_email != null || $check_email != ''){
            ...
            $this->db->insert('generalService', $page_data); //Inserts into the database.
            $generalService_id = $this->db->insert_id();
            move_uploaded_file($_FILES['file_name']['tmp_name'], 'uploads/generalService_image/' . $_FILES["file_name"]["name"]);
            //Uploads generalService's image into generalService image folder
            move_uploaded_file($_FILES['userfile']['tmp_name'], 'uploads/generalService_image/' . $generalService_id . '.jpg');
     ...
     //This will update the data entered in the form into the generalService database.
    function updategeneralServiceInformation($param2){      
        $page_data['name']              = html_escape($this->input->post('name'));
        $page_data['date_of_birth']     = strtotime($this->input->post('date_of_birth'));
        ...
        $this->db->where('generalService_id', $param2); //Will delete based on the department_id
        $this->db->update('generalService', $page_data); //Updates the database.

        move_uploaded_file($_FILES['userfile']['tmp_name'], 'uploads/generalService_image/' . $param2 . '.jpg');

    }

    function deletegeneralServiceInformation($param2){
        //This will delete the data entered in the form into the department database.
        $this->db->where('generalService_id', $param2); //Will delete based on the department_id
        $this->db->delete('generalService'); //Updates the database.
    }
