...
class Accountant_model extends CI_Model {
    ...
    function __construct() {
      ...
      //This function selects the accountant from the accountant table in the database.
    ...
    /*ends here */
     ...
     //This will insert the data entered in the form into the accountant table.
    function insertIntoAccountantTable(){   
        ...
        if($check_email != null || $check_email != ''){
            ...
            $this->db->insert('accountant', $page_data); //Inserts into the database.
            ...
            //Uploads accountant's image into accountant image folder
     ...
     //This will update the data entered in the form into the accountant database.
    function updateAccountantInformation($param2){      
        ...
        $this->db->where('accountant_id', $param2); //Will delete based on the department_id
        $this->db->update('accountant', $page_data); //Updates the database.

        move_uploaded_file($_FILES['userfile']['tmp_name'], 'uploads/accountant_image/' . $param2 . '.jpg');

    }

    function deleteAccountantInformation($param2){
        //This will delete the data entered in the form into the department database.
        $this->db->where('accountant_id', $param2); //Will delete based on the department_id
        $this->db->delete('accountant'); //Updates the database.
    }
