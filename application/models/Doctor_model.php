...
class Doctor_model extends CI_Model {
    ...
    function __construct() {
      ...
      //This function selects the doctor from the doctor table in the database.
    ...
    /*ends here */
     ...
     //This will insert the data entered in the form into the doctor table.
    function insertIntoDoctorTable(){   
        ...
        if($check_email != null || $check_email != ''){
            ...
            $this->db->insert('doctor', $page_data); //Inserts into the database.
            $doctor_id = $this->db->insert_id();
            move_uploaded_file($_FILES['file_name']['tmp_name'], 'uploads/doctor_image/' . $_FILES["file_name"]["name"]);
            //Uploads doctor's image into doctor image folder
     ...
     //This will update the data entered in the form into the doctor database.
    function updateDoctorInformation($param2){      
        ...
        $this->db->where('doctor_id', $param2); //Will delete based on the department_id
        $this->db->update('doctor', $page_data); //Updates the database.
    ...
    function deleteDoctorInformation($param2){
        ...
        $this->db->where('doctor_id', $param2); //Will delete based on the department_id
        $this->db->delete('doctor'); //Updates the database.
    }
