...
function validate(formData, jqForm, options) { 
	...
	$('#preloader-form').html('<img src="witty/images/preloader.gif" style="height:15px;margin-left:20px;" />');
...
{
	$('div.main_data').block({ message: '<img src="witty/images/preloader.gif" style="height:25px;" />' }); 
	$.ajax({
		url: url,
...
function delete_data(delete_url , post_refresh_url)
{
	...
	$('#preloader-delete').html('<img src="witty/images/preloader.gif" style="height:15px;margin-top:-10px;" />');
